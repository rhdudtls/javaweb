...
<head>
  ...
  <title>guestList(페이지 분량 조정하기).jsp</title>
...
<div class="container">
  ...
  <table class="table table-borderless mb-0 p-0">
    <tr>
      ...
      <td class="text-right">
      	...
      	<c:if test="${pag > 1}">
      		<a href="${ctp}/GuestList.gu?pag=1&pageSize=${pageSize}" title="첫페이지로">◁◁</a>
      		<a href="${ctp}/GuestList.gu?pag=${pag-1}&pageSize=${pageSize}" title="이전페이지로">◀</a>
      	</c:if>
      	${pag}/${totPage}
      	<c:if test="${pag < totPage}">
      		<a href="${ctp}/GuestList.gu?pag=${pag+1}&pageSize=${pageSize}" title="다음페이지로">▶</a>
      		<a href="${ctp}/GuestList.gu?pag=${totPage}&pageSize=${pageSize}" title="마지막페이지로">▷▷</a>
      	</c:if>
      </td>
    </tr>
  </table>
  ...
  <br/>	<!-- 4페이지에서 0블록으로 가게되면 현재페이지는 1페이지가 블록의 시작페이지 -->
  <!-- 첫페이지 / 이전블록 / 1(4) 2(5) 3 / 다음블록 / 마지막페이지 -->
  <div class="text-center">
  	<c:if test="${pag > 1}">[<a href="${ctp}/GuestList.gu?pag=1&pageSize=${pageSize}">첫페이지</a>]</c:if>
  	<c:if test="${curBlock > 0}">[<a href="${ctp}/GuestList.gu?pag=${(curBlock-1)*blockSize + 1}&pageSize=${pageSize}">이전블록</a>]</c:if>
  	<c:forEach var="i" begin="${curBlock*blockSize + 1}" end="${curBlock*blockSize + blockSize}">
  		<c:if test="${i <= totPage && i == pag}">[<font color="red">${i}</font>]</c:if>
  		<c:if test="${i <= totPage && i != pag}">[<a href="${ctp}/GuestList.gu?pag=${i}&pageSize=${pageSize}">${i}</a>]</c:if>
  	</c:forEach>
  	<c:if test="${curBlock < lastBlock}">[<a href="${ctp}/GuestList.gu?pag=${(curBlock+1)*blockSize + 1}&pageSize=${pageSize}">다음블록</a>]</c:if>
  	<c:if test="${pag < totPage}">[<a href="${ctp}/GuestList.gu?pag=${totPage}&pageSize=${pageSize}">마지막페이지</a>]</c:if>
