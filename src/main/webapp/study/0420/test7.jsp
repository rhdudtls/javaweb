...
<head>
  ...
  <script>
  	...
  	let idx = 0;
  	let newText = "";
  	//박스추가버튼을 클릭할때마다 입력폼 동적추가 : 각 TextField마다 고유한 id를 지정해준다. 
  	function inputBox(){
  	  idx++;
  	...
  	//상품 등록 전 기본 사항 체크 후 정상적인 값을 서버로 전송
  	function fCheck() {
  	  ...
  	  for(let i=0; i<products.length; i++) {
  		if(products[i].value == "") {
  		  alert("상품명을 입력하세요");
  		  document.getElementsByName("product"+i).focus();
  		  return false;
  		}
  		else if(prices[i].value == "") {
  		  alert("가격을 입력하세요")
  		  document.getElementsByName("price"+i).focus();
  		  return false; 		
  		}
  		else if(sus[i].value == "") {
  		  alert("수량을 입력하세요")
  		  document.getElementsByName("su"+i).focus();
  		  return false;
  		}
  	  }
  	  myform.submit();
