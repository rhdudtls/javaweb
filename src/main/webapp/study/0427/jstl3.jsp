...
<%
	...
	String[] hobbys = {"등산","낚시","수영","바둑","사이클","독서"};
...
%>
	<p>
		...
		4-3.문자열추출(substring()) : <%=atom.substring(2) %> / <%-- ${fn:substring(atom,2,5)} --%>
															${fn:substring(atom,2,fn:length(atom))} / 
															${fn:substring(atom,2,-1)}<br/>
		5-1.특정문자열의 위치값 검색(indexOf()) : <%=atom.indexOf("o")%> / ${fn:indexOf(atom,'o')}<br/>
		5-1.특정문자열의 위치값 검색(indexOf()) : <%=atom.lastIndexOf("o")%> / <%-- ${fn:indexOf(atom,'o') --%>}<br/>
		  atom변수에 기억되어 있는 'o'문자의 위치를 모두 출력해보자.<br/>
		  'o'의 위치는 =>
		<c:set var="cnt" value="0"/>
		<c:forEach var="i" begin="0" end="${fn:length(atom)-1}" step="1">
		...
		7.문자열분리(split(변수,분리할문자)) : 문자분리 후에는 분리된 문자들을 변수에 담아줘야 한다.<br/>
		...
		8.문자열 치환(replace())<br/>
		  예) atom변수의 'My'를 'Your'로 변경하시오?<br/>
		...
		9.특정문자(열) 포함유무? contains()<br/>
		  ...
		  예) atom2변수에 'o'를 포함하고 있느냐? ${fn:contains(atom2,'My')}<br/>
		  예) atom2변수에 'o'를 포함하고 있느냐? ${fn:contains(atom2,'Your')}<br/>
	</p>
</div>
<p><br/></p>
</body>
